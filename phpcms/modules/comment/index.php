...
class index {
	...
	public function post() {
		...
		$data = array('userid'=>$userid, 'username'=>$username, 'content'=>$content, 'direction'=>$direction);
				        //检查自定义字段值是否有添加！
				//水平凡 20111203
                $this->comment_plfielddb = pc_base::load_model('comment_plfield_model');
                $fies = $this->comment_plfielddb->select("", '*', 100, 'fid ASC');
                for($i=0;$i<count($fies);$i++){
                    $data[$fies[$i]['f']]=strip_tags($_POST[$fies[$i]['f']]);
                    if($fies[$i]['ismust']=="1"){
                        if(empty($_POST[$fies[$i]['f']])){
                            showmessage("信息项：".$fies[$i]['fname']."，没有填写！");
                        }
                    }
                }
