...
class content_tag {
	...
	public function lists($data) {
		$catid = intval($data['catid']);
		//造价天地栏目只显示manage001，和manage002添加的文章
		if(intval($_GET['catid']) === 19 || intval($_GET['ccu']) === 1){
			$username_where = ' AND username in("manage001","manage002","zjgly") ';
		}else{
			$username_where = '';
		}
		if(!$this->set_modelid($catid)) return false;
		if(isset($data['where'])) {
			...
			if($this->category[$catid]['child']) {
				...
				$pos = strpos($catids_str,',')+1;
				$catids_str = substr($catids_str, $pos);
				$sql = "status=99 ".$username_where." AND catid IN ($catids_str)".$thumb;
			} else {
				$sql = "status=99 ".$username_where." AND catid='$catid'".$thumb;
			}
		}
		$order = $data['order'];
